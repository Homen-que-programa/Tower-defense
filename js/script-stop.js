...
const tabelaCaminhoMetadeAzul = []
const tabelaCaminhoMetadeVermelho = []
...
function criarUnidadeAzul(x, y, vida, dano, danoV, velocidade, visao, range, tamanho, cor, espaco) {
    ...
    unidadeMensagemColisao.push((_colisao, _inimigo) => {
        ...
        if (_colisao && _inimigoIndex === -1) {
            ...
            if (_xposicaoTabelaIni - _xposicaoTabelaUnid !== 0) {
                ...
                let _copyxPosicaoTabelaUnid = _xposicaoTabelaUnid
                let _copyyPosicaoTabelaUnid = _yposicaoTabelaUnid
                if (_xposicaoTabelaUnid - _xposicaoTabelaIni > 0) {
                    for (let i = 0; i < Math.abs(_xposicaoTabelaUnid - _xposicaoTabelaIni); i++) {
                        _copyxPosicaoTabelaUnid--
                        _copyyPosicaoTabelaUnid = (_copyxPosicaoTabelaUnid * _a) + _b
                        if (tabelaCaminhoAzul[Math.floor(_copyyPosicaoTabelaUnid)][_copyxPosicaoTabelaUnid] !== '.') {
                ...
                let _copyxPosicaoTabelaUnid2 = _xposicaoTabelaUnid
                let _copyyPosicaoTabelaUnid2 = _yposicaoTabelaUnid
                if (_yposicaoTabelaUnid - _yposicaoTabelaIni > 0) {
                    for (let i = 0; i < Math.abs(_yposicaoTabelaUnid - _yposicaoTabelaIni); i++) {
                        _copyyPosicaoTabelaUnid2--
                        _copyxPosicaoTabelaUnid2 = (_copyyPosicaoTabelaUnid2 - _b) / _a
                        if (tabelaCaminhoAzul[_copyyPosicaoTabelaUnid2][Math.floor(_copyxPosicaoTabelaUnid2)] !== '.') {
                            _obstaculoCaminhoTrue = false
                        }
                    }
                } else {
                ...
                let _copyyPosicaoTabelaUnid = _yposicaoTabelaUnid
                for (let i = 0; i < Math.abs(_yposicaoTabelaUnid - _yposicaoTabelaIni); i++) {
                    _copyyPosicaoTabelaUnid++
                    if (tabelaCaminhoAzul[_copyyPosicaoTabelaUnid][_xposicaoTabelaUnid] !== '.') {
                        _obstaculoCaminhoTrue = false
                    }
                }
            }
    ...
    let _unidadeColisaoMovimento = () => {
        ...
        let _unidadeAlvoXY = [_alvoTabela[_monerDistancia[0]][7], _alvoTabela[_monerDistancia[0]][6]]
        psliderDist = ((_unidadeAlvoXY[1] - _x) ** 2 + (_unidadeAlvoXY[0] - _y) ** 2) ** 0.5
        if (_x === _unidadeAlvoXY[1]) {
            if (_y < _unidadeAlvoXY[0]) {
                _y += _unidadeVelocidade
            ...
            if (_xposicaoTabelaIni - _xposicaoTabelaUnid !== 0) {
                ...
                if (_xposicaoTabelaUnid - _xposicaoTabelaIni > 0) {
                    for (let i = 0; i < Math.abs(_xposicaoTabelaUnid - _xposicaoTabelaIni); i++) {
                        _copyxPosicaoTabelaUnid--
                        _copyyPosicaoTabelaUnid = (_copyxPosicaoTabelaUnid * _a) + _b
                        if (tabelaCaminhoVermelho[Math.floor(_copyyPosicaoTabelaUnid)][_copyxPosicaoTabelaUnid] !== '.') {
                            _obstaculoCaminhoTrue = false
                        }
                    }
                } else {
                ...
                if (_yposicaoTabelaUnid - _yposicaoTabelaIni > 0) {
                    for (let i = 0; i < Math.abs(_yposicaoTabelaUnid - _yposicaoTabelaIni); i++) {
                        _copyyPosicaoTabelaUnid2--
                        _copyxPosicaoTabelaUnid2 = (_copyyPosicaoTabelaUnid2 - _b) / _a
                        if (tabelaCaminhoVermelho[_copyyPosicaoTabelaUnid2][Math.floor(_copyxPosicaoTabelaUnid2)] !== '.') {
                            _obstaculoCaminhoTrue = false
                        }
                    }
                } else {
                ...
                for (let i = 0; i < Math.abs(_yposicaoTabelaUnid - _yposicaoTabelaIni); i++) {
                    _copyyPosicaoTabelaUnid++
                    if (tabelaCaminhoVermelho[_copyyPosicaoTabelaUnid][_xposicaoTabelaUnid] !== '.') {
                        _obstaculoCaminhoTrue = false
                    }
                }
            }
...
function atualizarCaminhoMetade() {
    ...
    for (let i = 0; i < tabelaCaminhoVermelho.length; i+=2) {
        ...
        for (let e = 0; e < tabelaCaminhoVermelho[i].length; e+=2) {
            if (tabelaCaminhoVermelho[i][e] !== '.' || tabelaCaminhoVermelho[i+1][e] !== '.' || tabelaCaminhoVermelho[i][e+1] !== '.' || tabelaCaminhoVermelho[i+1][e+1] !== '.') {
    ...
    for (let i = 0; i < _copyTabelaCaminhoMetade.length; i++) {
        tabelaCaminhoMetadeAzul[i] = Object.assign([], _copyTabelaCaminhoMetade[i])
    }
    for (let i = 0; i < _copyTabelaCaminhoMetade.length; i++) {
        tabelaCaminhoMetadeVermelho[i] = Object.assign([], _copyTabelaCaminhoMetade[i])
    }
}
atualizarCaminhoMetade()
...
//     t
// atualizarCaminhoMetade()abelaCaminhoVermelho[9][22] = 'N'
//     caminhoAlteracoes++
// }, 500)
